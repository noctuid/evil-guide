...
The state of window-configuration libraries has always been a bit compliated in Emacs. Emacs now has ~tab-line-mode~ and ~tab-bar-mode~, so use those if they work for you.
...
The messages buffer requires special handling. Using =messages-buffer-mode-hook= will not work (possibly because the messages buffer exists so early on). You need to add a hook to run it later. ~general-add-hook~ is used here for the transient feature (this function will remove itself from =post-command-hook= after the first run in the messages buffer):
#+begin_src emacs-lisp
(with-eval-after-load 'evil
  (general-add-hook 'post-command-hook
                    ...
                    nil
                    nil
                    ;; transient; remove self on success
                    #'identity))
#+end_src
